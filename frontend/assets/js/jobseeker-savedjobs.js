...
window.onload = function() {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            xhttp1.open(
                "POST",
                "http://localhost/metierfind/backend/api/savedjobs/fetchAllSaved.php"
            );
            ...
            xhttp1.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (let row of results) {
                        ...
                        document.getElementById("fetch-saved").insertAdjacentHTML(
                            ...
                            <div class="col-30">
                                ...
                                <div class="buttons">
                                    <button class="apply-job-btn" id="${row.j_id}" onclick="ApplyJob()">Apply Now</button><br>
                                    <button class="remove-job-btn" id="${row.j_id}" onclick="RemoveJob()">Remove</button>
                                </div>
                            </div>
                            </div>
                            `
                    ...
                    //     xhttp1.open(
                    //         "POST",
                    //         "http://localhost/metierfind/backend/api/jobs/fetchAllSaved.php"
                    //     );
                    //     xhttp1.send(JSON.stringify({ j_id: row.j_id }));
                    ...
                    //                 );
                    //                 xhttp2.send(JSON.stringify({ employerid: row.c_employerid }));
                    ...
                    //                                 <img src="${results.c_photo}" width="200px" height="200px">
                    //                                 <h1>${results.c_name}</h1>
                    //                                 <h2>${results.c_location}</h2>  
                    //                                 <p><i class="fa fa-clipboard"></i>${row.j_posted_date}</p>
                    //                                 <p><i class="fa fa-globe "></i>${results.c_website}</p>
                    //                                 <p><i class="fa fa-search"></i>More Jobs at ${results.c_website} Inc</p>
                    //                                 <h1 class="salary" style="color: #ff3e97;">Php ${row.j_salary}</h1>
                    //                                 <div class="buttons">
                    //                                     <button class="apply-job-btn" id="${row.j_id}">Apply Now</button><br>
                    //                                     <button class="remove-job-btn" id="${row.j_id}">Remove</button>
                    //                                 </div>
                    //                             </div>
                    //                         `
                    //                         );
...
function RemoveJob() {
    ...
    if (confirm('Are you sure you want to remove this job ?')) {
        ...
        xhttp.open(
            "DELETE",
            "http://localhost/metierfind/backend/api/savedjobs/delete.php"
        );
